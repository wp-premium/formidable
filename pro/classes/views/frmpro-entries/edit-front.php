...
<?php
    } else if ( ( isset($jump_to_form) && $jump_to_form) || ! empty($message) ) {
        FrmFormsHelper::get_scroll_js($form->id);
    }
?>
<form enctype="multipart/form-data" method="post" class="frm-show-form <?php do_action('frm_form_classes', $form) ?>" id="form_<?php echo esc_attr( $form->form_key ) ?>" <?php echo $frm_settings->use_html ? '' : 'action=""'; ?>>
<?php
    $form_action = 'update';
