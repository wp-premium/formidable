<div id="frm_watch_lookup_<?php echo esc_attr( $field_id .'_'. $row_key ) ?>">
	<select name="field_options[watch_lookup_<?php echo esc_attr( $field_id ) ?>][]">
	    ...
	    ?>
	    <option value="<?php echo esc_attr( $field_option->id ) ?>"<?php echo esc_attr( $selected ) ?>><?php echo ('' == $field_option->name) ? $field_option->id .' '. __( '(no label)', 'formidable-pro' ) : esc_html( $field_option->name );
	    ?></option>
	    <?php } ?>
	</select>
