<div class="manage-menus">
	<span class="add-edit-menu-action">
		...
		if ( count( $styles ) < 2 && ! empty( $style->ID ) ) {
			printf(
				__( 'Edit your style below, or %1$screate a new style%2$s or %3$sduplicate the current style%4$s.', 'formidable' ),
			...
			<span class="add-new-menu-action">
				...
				if ( empty( $style->ID ) ) {
					printf(
						__( 'or %1$screate a new style%2$s', 'formidable' ),
						'<a href="?page=formidable-styles&frm_action=new_style">', '</a>'
					);
				} else {
					printf(
						__( 'or %1$screate a new style%2$s or %3$sduplicate the current style%4$s.', 'formidable' ),
						'<a href="?page=formidable-styles&frm_action=new_style">', '</a>',
						'<a href="?page=formidable-styles&frm_action=duplicate&style_id=' . absint( $style->ID ) . '">','</a>'
					);
				} ?>
