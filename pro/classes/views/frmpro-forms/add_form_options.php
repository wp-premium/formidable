<h3><?php _e( 'Permissions', 'formidable-pro' ) ?>
...
<tr>
    ...
    <td class="td_select_padding">
        <select name="options[logged_in_role]" id="logged_in_role" class="hide_logged_in <?php echo esc_attr( $values['logged_in'] ? '' : 'frm_invisible' ); ?>">
            <option value=""><?php _e( 'Logged-in Users', 'formidable-pro' ) ?></option>
...
<tr>
    ...
    <td class="td_select_padding">
        <select name="options[single_entry_type]" id="frm_single_entry_type" class="hide_single_entry <?php echo esc_attr( $values['single_entry'] ? '' : 'frm_invisible' ); ?>">
            <option value="user" <?php selected($values['single_entry_type'], 'user') ?>><?php _e( 'Logged-in User', 'formidable-pro' ) ?></option>
			<?php if ( FrmAppHelper::ips_saved() ) { ?>
            <option value="ip" <?php selected($values['single_entry_type'], 'ip') ?>><?php _e( 'IP Address', 'formidable-pro' ) ?></option>
			<?php } ?>
            <option value="cookie" <?php selected($values['single_entry_type'], 'cookie') ?>><?php _e( 'Saved Cookie', 'formidable-pro' ) ?></option>
        </select>
    </td>
</tr>
<tr id="frm_cookie_expiration" class="frm_short_tr <?php echo ( $values['single_entry'] && $values['single_entry_type'] == 'cookie' ) ? '' : 'frm_hidden'; ?>">
    <td colspan="2">
        <p class="frm_indent_opt">
            ...
            <input type="text" name="options[cookie_expiration]" value="<?php echo esc_attr($values['cookie_expiration']) ?>"/> <span class="howto"><?php _e( 'hours', 'formidable-pro' ) ?></span>
        </p>
    </td>
</tr>
...
<tr>
    <td colspan="2">
		<label for="editable">
			...
			<?php _e( 'Allow front-end editing of entries', 'formidable-pro' ) ?>
		</label>
    </td>
</tr>
...
<tr class="hide_editable frm_short_tr">
    <td>
        <p class="frm_indent_opt"><label for="editable_role"><?php _e( 'Role required to edit one\'s own entries:', 'formidable-pro' ) ?></label></p>
    </td>
    <td>
	...
	<td>
		<p class="frm_indent_opt">
			<label for="open_editable_role">
				<?php _e( 'Role required to edit other users\' entries:', 'formidable-pro' ) ?>
			</label>
		</p>
	</td>
    <td>
        <select name="options[open_editable_role]" id="open_editable_role">
			...
			<option value="" <?php selected( $values['open_editable_role'], '' ); ?>>
				<?php _e( 'Logged-in Users', 'formidable-pro' ) ?>
			</option>
...
<tr class="hide_editable frm_short_tr">
    <td colspan="2">
        <div class="frm_indent_opt"><label><?php _e( 'On Update:', 'formidable-pro' ) ?></label></div>
        <span class="frm_indent_opt alignleft" style="width:200px;">
            <select name="options[edit_action]" id="edit_action">
                <option value="message" <?php selected( $values['edit_action'], 'message' ) ?>><?php _e( 'Show Message', 'formidable-pro' ) ?></option>
				<option value="redirect" <?php selected( $values['edit_action'], 'redirect' ) ?>><?php _e( 'Redirect to URL', 'formidable-pro' ) ?></option>
				<option value="page" <?php selected( $values['edit_action'], 'page' ) ?>><?php _e( 'Show Page Content', 'formidable-pro' ) ?></option>
            </select>
        </span>
		...
		<span class="edit_action_page_box edit_action_box <?php echo esc_attr( $values['edit_action'] == 'page' ? '' : 'frm_hidden' ); ?>">
            <label><?php _e( 'Use Content from Page', 'formidable-pro' ) ?></label>
...
<tr>
    <td colspan="2">
		<label for="save_draft">
			...
			<?php _e( 'Allow logged-in users to save drafts', 'formidable-pro' ) ?>
		</label>
    </td>
</tr>
	...
	<tr>
	    <td colspan="2">
			<label for="protect_files">
				...
				<?php _e( 'Protect all files uploaded in this form', 'formidable-pro' ) ?>
			</label>
	    </td>
	</tr>
<?php } else { ?>
