<div class="misc-pub-section">
    ...
    <div id="frm_shortcodediv" class="hide-if-js">
		...
		<p><input type="text" style="width:98%;" readonly="readonly" class="frm_select_box" value="[display-frm-data id=<?php echo ( isset( $post->ID ) ) ? $post->ID : __( 'Save to get ID', 'formidable' ) ?> filter=1]" />
    	<?php if ( isset($post->post_name) && ! empty( $post->post_name ) && ( $post->post_name != $post->ID ) ) { ?>
		<input type="text" style="width:98%;margin-top:4px;" readonly="readonly" class="frm_select_box" value="[display-frm-data id=<?php echo ( isset( $post->post_name ) && $post->post_name != '' ) ? $post->post_name : '??'; ?> filter=1]" />
    	<?php } ?>
    	</p>

    	<p class="howto"><?php _e( 'Insert in a template', 'formidable' ) ?>:</p>
		<p><input type="text" style="font-size:10px;width:98%;font-weight:normal" readonly="readonly" class="frm_select_box" value="&lt;?php echo FrmProDisplaysController::get_shortcode( array( 'id' => <?php echo ( isset( $post->ID ) ) ? $post->ID : '??' ?>) ) ?&gt;" /></p>
