...
<label class="frm_left_label"><?php _e( 'Event Date', 'formidable' ); ?></label>
...
<select id="edate_field_id" name="options[edate_field_id]">
    ...
    <?php
	if ( is_numeric( $post->frm_form_id ) && ! empty( $post->frm_form_id ) ) {
		FrmProFieldsHelper::get_field_options( $post->frm_form_id, $post->frm_edate_field_id, '', array( 'date', 'number', 'select', 'radio', 'scale' ) );
    } ?>
</select>
<br/>

<label class="frm_left_label"><?php _e( 'Repeat', 'formidable' ); ?> <span class="frm_help frm_icon_font frm_tooltip_icon" title="<?php printf(__( 'Select a field from your form that contains values like 1 week, 2 weeks, 1 year, etc. This will set the repeat period for each event.', 'formidable' ), FrmAppHelper::site_url()) ?>" ></span> </label>
<select id="repeat_event_field_id" name="options[repeat_event_field_id]">
    <option value=""><?php _e( 'No repeating events', 'formidable' ) ?></option>
    <?php
    if ( is_numeric($post->frm_form_id) && !empty($post->frm_form_id) ) {
...
<label class="frm_left_label"><?php _e( 'End Repeat', 'formidable' ); ?></label>
<select id="repeat_edate_field_id" name="options[repeat_edate_field_id]">
    <option value=""><?php _e( 'Never', 'formidable' ) ?></option>
    <?php
	if ( is_numeric( $post->frm_form_id ) && ! empty( $post->frm_form_id ) ) {
