<tr class="show_repeat_sec">
	<td><label><?php _e( 'Repeat Layout', 'formidable-pro' ) ?></label></td>
	<td>
		<select name="field_options[format_<?php echo absint( $field['id'] ) ?>]">
			<option value=""><?php _e( 'Default: No automatic formatting', 'formidable-pro' ) ?></option>
			<option value="inline" <?php selected($field['format'], 'inline') ?>><?php _e( 'Inline: Display each field and label in one row', 'formidable-pro' ) ?></option>
			<option value="grid" <?php selected($field['format'], 'grid') ?>><?php _e( 'Grid: Display labels as headings above rows of fields', 'formidable-pro' ) ?></option>
		</select>
		<input type="hidden" name="field_options[form_select_<?php echo absint( $field['id'] ) ?>]" value="<?php echo esc_attr( $field['form_select'] ) ?>" />
	</td>
</tr>
<tr class="show_repeat_sec">
    <td><label><?php _e( 'Repeat Limit', 'formidable-pro' ) ?></label>
